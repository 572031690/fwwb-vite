...
import { routerList, staticRoute } from '../../assets/data/homeRouter'
...
export default {
    ...
    setup() {
        ...
        window.sessionStorage.setItem('permissionName',JSON.stringify(staticRoute))
        window.sessionStorage.setItem('storeData','小明')
        data.permissionName = JSON.parse(
        ...
        const getAdminType = () => {
            ...
            for (let i = 0; i < data.routerList.length; i++) {
                if (
                constrolPermission.includes(data.routerList[i].permissionName as string) &&
                data.routerList[i].type === 'router'
        ...
        const changearrow = (index:number) => {
            ...
            if (!data.routerList[index].navSonShow) {
                data.arrowData[index].style.transform =
                'rotate(270deg)'
