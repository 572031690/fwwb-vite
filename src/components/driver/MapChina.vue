...
export default {
    ...
    setup(props: any, content: any) {
        ...
        // const modules:any = {}
        // 读取土壤-图层的图片文件
        const province: any = import.meta.glob('../../../public/static/map/province/*.json')
        // console.log(province,'province');
        // for (const key in province) {
        //     const keys:string = key.replace(/(\.\/module\/|\.json)/g, '') // 去除末尾的.json
        //     modules[keys.substring(keys.lastIndexOf("/")+1)] = province[key].default
        // }
        ...
        const getMap = async () => {
            ...
            const contexts = import.meta.glob("../../../public/static/map/china.json")
            // console.log(contexts["../../../public/static/map/china.json"],'contexts');
            contexts["../../../public/static/map/china.json"]().then((res: any) => {
                console.log(res, 'res');
                echarts.registerMap('chinaMap', res.default) // 注册map数据
                // 绘制图表
                const setOption = {
                    tooltip: {
                        // 鼠标一上去的信息框
        ...
        const setOnClick = async () => {
            ...
            data.chartInstance.on('click', async (arg: { name: string, componentSubType: string }) => {
                ...
                if (!data.mapData[provinceInfo.key]) {
                    ...
                    console.log(provinceInfo.path, 'provinceInfo.key');
                    console.log(province, 'provinceInfo.key');
                    console.log(province[provinceInfo.path], 'provinceInfo.key');
                    province[provinceInfo.path]().then((res: any) => {
                        console.log('object');
                        data.mapData[provinceInfo.key] = res.default
                        echarts.registerMap(provinceInfo.key, res.default)
                        const changeOption = {
                            geo: [
                                {
